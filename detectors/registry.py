...
DETECTORS = {
    ...
    "keyword-baseline": {
        ...
        "methodology": (
            "工业级规则基线 v2：归一化管道（全半角/零宽/繁简/中文拆字压缩）"
            "+ 谐音黑话映射（菠菜→博彩等）+ 五类词库（赌博/色情/诈骗/毒品/武器）"
            "+ 12 条注入指令正则 + 外挂词库接口（wordlists/extra_*.txt）。"
            "代表平台关键词第一道防线；规则系统的天花板：看得见词法看不见语义——"
            "拼音缩写/语义伪装天然盲区，用它验证'反馈泄漏逐条绕过'的脆弱性。"),
    },
}
